...
func initClaimRoutes(r *gin.RouterGroup, svcEps v1.Endpoints) {
	r.GET("/getList", func(c *gin.Context) {
		list := dtoV1.RequestClaimListFilters{}
		...
		resp, err := svcEps.GetClaimsList(newCtx, list)
		if err != nil {
	...
	r.GET("/:byID", func(c *gin.Context) {
		...
		// вызываем конечную точку обработки запроса
		resp, err := svcEps.GetClaimByID(newCtx, reqID)
		if err != nil {
			log.Println("ClaimByID router err:", err)
			c.JSON(resp.CodeErr, resp)
			return
		}
	...
	r.POST("/store", func(c *gin.Context) {
		...
		// вызываем конечную точку обработки запроса
		resp, err := svcEps.ClaimStore(newCtx, data)
		if err != nil {

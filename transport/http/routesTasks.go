...
func initTaskRoutes(r *gin.RouterGroup, svcEps v1.Endpoints) {
	r.GET("/getList", func(c *gin.Context) {
		list := dtoV1.RequestTaskListFilters{}
		...
		resp, err := svcEps.GetTasksList(newCtx, list)
		if err != nil {
	...
	r.GET("/:byID", func(c *gin.Context) {
		...
		// вызываем конечную точку обработки запроса
		resp, err := svcEps.GetTaskByID(newCtx, reqID)
		if err != nil {
	...
	r.POST("/store", func(c *gin.Context) {
		...
		// вызываем конечную точку обработки запроса
		resp, err := svcEps.TaskStore(newCtx, data)
		if err != nil {
